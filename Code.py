...
td = [1.20, 0.10, 0.60]                  # The perception-reaction time of manually driven vehicle, CACC vehicle, ACC vehicle

'''==Parameters for transition mode=='''
KT_CA = 15                               # The duration of transition from CACC mode to ACC mode
KT_AC = 15                               # The duration of transition from ACC mode to CACC mode
...
def ACC(back_veh, fore_veh, t_acc):
    ...
    if ind == 1:
        if s <= thw_acc * initial_speed * 1.20:               # gap-regulating mode
            a_ACC = k1 * (fore_veh[0] - back_veh[0] - t_acc * back_veh[1] - len_veh) + k2 * (fore_veh[1] - back_veh[1])
            nextind = 1 
        else:                                                 # controller switch from gap-regulating mode to gap-closing mode
            a_ACC = k1_c * (fore_veh[0] - back_veh[0] - t_acc * back_veh[1] - len_veh) + k2_c * (fore_veh[1] - back_veh[1])
            nextind = 2
    else:
...
def CACC(back_veh, fore_veh, t_cacc):
    ...
    if ind == 1:
        if s <= thw_cacc * initial_speed * 1.20:             # gap-regulating mode
            dv_CACC = kp * gaperror + kd * (fore_veh[1] - back_veh[1] - t_cacc * back_veh[2])
            nextind = 1
        else:                                                # controller switch from gap-regulating mode to gap-closing mode
            dv_CACC = kp_c * gaperror + kd_c * (fore_veh[1] - back_veh[1] - t_cacc * back_veh[2])
            nextind = 2
    else:
        if gaperror < errorthre:                             # gap closing mode
            dv_CACC = kp * gaperror + kd * (fore_veh[1] - back_veh[1] - t_cacc * back_veh[2])
...
def MDV(back_veh, fore_veh):
    temp1 = back_veh[1] * thw_mdv + (back_veh[1] * (back_veh[1] - fore_veh[1]) / 2 / math.sqrt(max_a * b))
    s1 = s0 + max([0, temp1])
    temp2 = max_a * (1 - (back_veh[1] / v0) ** 4 - (s1 / (fore_veh[0] - back_veh[0] - len_veh)) ** 2 )
...
def TransitionGap (mode1, mode2, t):                        # Transition from mode1 to mode2
...
def LaneUpdation(lane, ind):
    update_dv = [0 for i in range(len(lane))]                # Store the updated speed change (acceleration * simulation step) 
    if ind != 0:
        for i in range(len(lane) - 1):
            if lane[i][0] >= set_area[0] and lane[i][0] <= set_area[1] and lane[i][3] == 1:  # The degradation from CACC mode to ACC mode
                lane[i][4] = 2
                lane[i][5] += simu_step
            if lane[i][0] > set_area[1] and lane[i][3] == 1:                                 # The upgradation from ACC mode to CACC mode 
                lane[i][4] = 1
                lane[i][6] += simu_step                   
    ...
    for i in range(len(lane) - 1):
        ...
        if lane[i][3] == 2:                                 # Update the dynamics of initial ACC vehicle
...
def Fleet(lane):
    for runtime in range(6000):                             # The preheating time is 600s
        LaneUpdation(lane, 0)
    start_x = lane[-1][0]
    for veh in lane:                                        # Initialize vehicle position, make the first vehicle located at the start point of simulation segment
        veh[0] = veh[0] - start_x
    return lane
...
def CalIndex(line):
    ...
    for i in range(len(line)-1):
        ind = line[i][4]
        SSDL = line[i + 1][0] - line[i][0] - len_veh + (line[i + 1][1]) ** 2 / 2 / dm
...
def VehSD(Speed_list):
    ...
    with open("SD_ALL.txt", "w") as SDtoTXT:
        for j in range(10):
            unit_speed = [Speed_list[j][i] for i in range(len(Speed_list[1]))]            # Extract the speed of each vehicle
...
def VehAccele(Accele_list):
    ...
    with open("AC_ALL.txt", "w") as SDtoTXT:
        for j in range(10):
            unit_accele = [abs(Accele_list[j][i]) for i in range(len(Accele_list[1]))]   # Extract the absolute acceleration of each vehicle
...
def Toexcel(name, data):
    ...
    matrix.columns = ["1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th", "10th"]
    matrix.to_excel(name)

'''==Total simulation process=='''
def SingleProcess():
    TERC_all = [0 for i in range(10)]                            # Initialize lists
    ...
    lane = Initial()                                             # Generate a platoon
    fleet = Fleet(lane)                                          # Preheating simulation
    Write_txt("0_lane.txt", fleet)                               # Output the initial platoon
    
    while (fleet[-1][0] < len_link):                             # Extract the basic variables
    ...
    Write_txt("1_lane.txt", fleet)                              # Output the platoon after simulation

    print("Average TERCRI: %f" %float(np.average(TERC_all)))    # Calculate and print the surrogate safety measures
    print("Average TIT: %f" %float(np.average(TIT_all)))
    VehSD(Speed)
    VehAccele(Acceleration)
    
    Write_txt("TIT_ALL.txt", TIT_all)                           # Write the results to files
...
if __name__ == "__main__":
    mdv = []                                                    # input “-n”, represting the nth vehicle is MDV; Empty list represents the full CACC platoon 
    improve = 0                                                 # The index respresent whether the MDV equips information-sending devices, 0-no, 1-yes            
